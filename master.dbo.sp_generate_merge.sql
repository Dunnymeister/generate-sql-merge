...
IF (@nologo = 0)
BEGIN
 SET @output += @b + '--MERGE/DELETE generated by ''sp_generate_merge'' stored procedure, Version 0.93'
...
-- If no rows were returned, and we don't want to delete any rows, then terminate early
IF ((SELECT COUNT(*) FROM @tab) <= 0 AND @delete_if_not_matched = 0)
BEGIN
	set @output += @b + '--No rows to MERGE found and @delete_if_not_matched = 0 so not generating a delete statement. No work to do - exiting!'
	SELECT @output
	RETURN 0
END

DECLARE @statement_type varchar(6);
IF (SELECT COUNT(*) FROM @tab) <= 0 -- No rows returned, let's change our approach since an empty merge statement is invalid SQL
BEGIN
	-- We already checked if @delete_if_not_matched = 0 earlier, no need to repeat
	-- Let's delete all rows in the destination table
	SET @output = @b + 'DELETE FROM ' + @Target_Table_For_Output
	SET @statement_type = 'DELETE'
END
ELSE
BEGIN
	SET @statement_type = 'MERGE'
...
BEGIN
 ...
 SET @output += @b + ' BEGIN'
 SET @output += @b + ' PRINT ''ERROR OCCURRED IN ' + @statement_type + ' FOR ' + @Target_Table_For_Output + '. Rows affected: '' + CAST(@mergeCount AS VARCHAR(100)); -- SQL should always return zero rows affected';
 SET @output += @b + ' END'
 SET @output += @b + 'ELSE'
 SET @output += @b + ' BEGIN'
 SET @output += @b + ' PRINT ''' + @Target_Table_For_Output + ' rows affected by ' + @statement_type + ': '' + CAST(@mergeCount AS VARCHAR(100));';
 SET @output += @b + ' END'
 SET @output += @b + @batch_separator
 SET @output += @b + @b
END

-- We only disabled the constraints if we wrote out a merge statement
-- Check if we need to re-enable them
...
BEGIN
	...
	PRINT @statement_type + ' statement has been wrapped in an XML fragment and output successfully.'
